...
//product status
    ...
    //product search
    @RequestMapping(value = "/api/rfx/company/capabilities/products/search", method = RequestMethod.POST)
	...
	public ResponseEntity<?> deleteCompanyProductModule(@RequestParam("productModuleId") int productModuleId){
		...
		{       logger.info("Deleted products module..");
			...
			return getErrorResponseEntity(Constants.CONTACT_ADMIN_COMPNY_CAPABIL, Constants.ERROR_CODE_403, HttpStatus.FORBIDDEN);
		}
	}
...
//product module active
   ...
   //product module get 
    ...
    public  ResponseEntity<?>  getProductModuleList() throws Exception 
    {
    	if (rfxAdminService.checkRoleAsAdmin()) 
    	{
    		logger.info("Getting Product Module..");
    		return ResponseEntity.ok(rfxAdminService.getProductModuleList());
    	}else {
    		return getErrorResponseEntity(Constants.CONTACT_ADMIN_COMPNY_CAPABIL, Constants.ERROR_CODE_400, HttpStatus.BAD_REQUEST);
    	}
